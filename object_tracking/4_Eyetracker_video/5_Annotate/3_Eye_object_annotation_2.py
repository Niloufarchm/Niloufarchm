...
def add_object_positions_to_files(base_path, files, object_file, threshold_ns=42e6):
    ...
    for file in files:
        ...
        # Use the 'start timestamp [ns]' field from annotated data directly
        updated_data = add_object_positions(annotated_data, object_positions, 'start timestamp [ns]', 'timestamp [ns]', threshold_ns)
...
def add_object_positions(annotated_data, object_positions, annotated_timestamp_field, object_timestamp_field, threshold):
...
base_path = ''
files = ['annotated_saccades.csv', 'annotated_fixations.csv', 'annotated_blinks.csv']
